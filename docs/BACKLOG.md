...
Last updated: 2026-04-11 - Batch 20 shipped: **GIS analyst tools — layer manager, identify, go-to, measurements** — `src/components/Globe/DataLayers.tsx` fully rewritten as a proper layer manager with compact `BuiltinLayerRow` (eye toggle, drag-reorder via HTML5 drag, per-layer opacity slider), `CustomLayerRow` (up/down reorder, visibility, opacity, remove), and `AddLayerForm` supporting WMS/WMTS/XYZ tile URLs; `CustomLayer`/`CustomLayerType` types added to `src/types/index.ts`; `createCustomLayer` factory in `src/lib/map-layers.ts`; custom layers render on Cesium via `WebMapServiceImageryProvider`/`WebMapTileServiceImageryProvider`/`UrlTemplateImageryProvider` in a new `useEffect` in `CesiumGlobe.tsx`; active layer count badge on the Layers button; **Feature Inspector** — new `src/components/Globe/FeatureInspectorPanel.tsx` with `IdentifyHit`/`IdentifyResult` types; `identifyMode`/`identifyResult` state in `useExploreState`; identify click handler in `CesiumGlobe` picks entities (drawn shapes, sites, fires, earthquakes) and imagery layers via `pickImageryLayerFeatures` (WMS GetFeatureInfo); FEMA flood layer `enablePickFeatures` flipped to `true`; crosshair cursor in identify mode; floating inspector panel with type badges (WMS/Feature/AOI/Site/Fire/Quake) and striped attribute table; **Go-to Coordinates** — new `src/components/Globe/GoToCoordinateDialog.tsx`; `Ctrl+G`/`Cmd+G` shortcut; parses decimal degrees, DMS (with symbols or spaces), DDM, and auto-detects swapped lng/lat; live preview with DMS conversion; quick-jump buttons (Prime Meridian, Null Island, poles); **Enhanced measurements** — `DrawnMeasurement` extended with `distanceKm/Mi`, `areaAcres/Ha/Km2/Mi2`, `perimeterKm/Mi`, `bearingDeg/Display`; `computeBearing` and `computePerimeter` helpers in `analysis-geometry.ts` using `@turf/turf` bearing; `MeasurementDetail` component in `AoiDrawingToolbar.tsx` shows dual-unit readouts + compass bearing for lines, four-unit area + perimeter for polygons; keyboard shortcut `I` toggles identify mode.
...
- ~~Layer manager with custom WMS/WMTS/XYZ~~ — **shipped**: `DataLayers.tsx` rewritten with drag-reorder, per-layer opacity, `AddLayerForm` for external WMS/WMTS/XYZ URLs, custom layers render via Cesium imagery providers
- ~~Feature Inspector / Identify tool~~ — **shipped**: `FeatureInspectorPanel.tsx`; identify mode picks entities + WMS GetFeatureInfo; floating panel with type badges and attribute table
- ~~Go-to Coordinates dialog~~ — **shipped**: `GoToCoordinateDialog.tsx`; `Ctrl+G` opens modal; parses decimal/DMS/DDM; live preview; quick-jump buttons
- ~~Enhanced measurements~~ — **shipped**: bearing/azimuth on lines, multi-unit area (ac/ha/km²/mi²), perimeter for polygons, dual-unit distance (mi/km)
- WMS GetCapabilities browser (auto-discover available layers from a WMS URL)
- Attribute table for drawn features + nearby places (select row → highlight on globe)
- Layer legend panel with WMS `GetLegendGraphic` support
