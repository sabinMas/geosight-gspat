...
Last updated: 2026-04-16 - Doc reconciliation: all five explorer-mode lenses (`hunt-planner`, `trail-scout`, `land-quick-check`, `road-trip`, `general-explore`) now have deterministic analyzers + prompt builders wired through `src/app/api/lens-analysis/route.ts`. Remaining analyzer gap is Pro-profile lenses. Other open "next pickup" items: (a) debounced viewport-based overlay refresh + zoom gating so roads / fires / flood layers do not over-fetch while panning, (b) save/share/export batch on top of the shared `LayerState` + AOI GeoJSON + deterministic lens result contracts, (c) first live non-US provider integration (GloFAS global flood is the cleanest candidate), (d) GeoAnalyst system-prompt context audit for recently added `GeodataResult` fields.
...
- ~~Live non-US flood provider~~ — **shipped**: GloFAS (Global Flood Awareness System) via Open-Meteo flood API, wired in `src/lib/fema-flood.ts` as a global fallback to FEMA NFHL; `FloodZoneResult.source === "glofas"` branch surfaces ensemble river-discharge forecasts in `FloodRiskCard.tsx`, GeoAnalyst `buildSupportedFacts`, and the `land-quick-check` lens analyzer.
- Live non-US provider integrations remaining: soil (ISRIC World Soil Information), seismic (GEM OpenQuake), school data beyond Eurostat (OSM + UNESCO density).
