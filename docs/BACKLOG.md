...
Last updated: 2026-04-20 — Competition polish batch shipped: (1) **Automated demo system** — `src/components/Demo/DemoRunner.tsx` (new) + `src/lib/demo-scenarios.ts` (new); 3 guided interactive tours (Home Buyer / Boulder CO, Data Center / Phoenix AZ, Trail Scout / Yosemite CA); landing page "Watch a demo" button opens a picker modal (3 scenario cards); each tour navigates to `/explore?demo=<id>&lat=&lng=&...`; `DemoRunner` renders a fixed bottom-center panel with spotlight ring (`getBoundingClientRect` on `data-demo-id` elements), `requestAnimationFrame` progress bar, step dots, Next/Done/Stop buttons; `dataReady` gate prevents advancing past intro before data loads; `demoScenarioId` added to `ExploreInitState` + parsed in `explore/page.tsx`; `buildExploreHref` extended with `lat`, `lng`, `demoScenarioId` params. (2) **Compare button UX** — disabled when `sites.length < 2`, count badge (`X saved`), `Columns2` icon, dynamic tooltip. (3) **Design audit fixes** — `SchoolContextCard` school name truncation (`min-w-0` + `truncate`); `ImageUpload` land-cover bucket colors from hardcoded hex → CSS tokens (`var(--color-success)`, `var(--accent)`, `var(--muted-foreground)`, `var(--color-warning)`); `ChatPanel` submit button `rounded-full`; "Save as new" / "Update active" / "Save site" buttons `whitespace-nowrap shrink-0`.
