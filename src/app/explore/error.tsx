...
export default function ExploreError({
  ...
  useEffect(() => {
    console.error("[GeoSight][ExploreError]", error);
  }, [error]);
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[var(--background)] px-6 py-16 text-[var(--foreground)]">
      <div className="w-full max-w-xl rounded-[2rem] border border-[color:var(--border-soft)] bg-[var(--surface-raised)] p-8 shadow-[0_24px_80px_rgba(0,0,0,0.28)]">
        ...
        <h1 className="mt-3 text-3xl font-semibold tracking-tight">GeoSight hit a runtime issue.</h1>
        <p className="mt-4 text-sm leading-7 text-[var(--muted-foreground)]">
          Live data can occasionally arrive in an unexpected shape. The workspace did not load
          cleanly, but you can retry without losing the rest of the application.
        </p>
