import { getLensLabel, normalizeProfileId } from "@/lib/lenses";
import { MissionProfile } from "@/types";

export const PROFILES: MissionProfile[] = [
  {
    id: "data-center",
    name: getLensLabel("data-center"),
    icon: "Server",
    tagline: "Water, power, and thermal efficiency",
    description:
      "Evaluate sites for data center cooling with an emphasis on water access, power infrastructure, terrain, and climate.",
    accentColor: "#00e5ff",
    factors: [
      {
        key: "waterProximity",
        label: "Cooling water access",
        weight: 0.26,
        scoreFn: "custom",
        params: { metric: "waterAccess" },
        description: "Combines mapped water proximity with nearby USGS stream gauge distance and discharge volume.",
      },
      {
        key: "terrain",
        label: "Elevation & flatness",
        weight: 0.12,
        scoreFn: "elevation",
        params: { mode: "cooling" },
        description: "Flatter, lower-elevation sites are easier to develop and cool.",
      },
      {
        key: "powerInfrastructure",
        label: "Power infrastructure",
        weight: 0.18,
        scoreFn: "distance",
        params: { source: "power", idealKm: 2, cutoffKm: 20, direction: "near" },
        description: "Close proximity to substations or transmission corridors reduces utility friction.",
      },
      {
        key: "climate",
        label: "Climate suitability",
        weight: 0.13,
        scoreFn: "climate",
        params: { mode: "cooling" },
        description: "Cooler average temperatures and lower cooling demand improve suitability.",
      },
      {
        key: "transportation",
        label: "Road transportation",
        weight: 0.07,
        scoreFn: "distance",
        params: { source: "road", idealKm: 2, cutoffKm: 18, direction: "near" },
        description: "Closer road access helps with construction and operations logistics.",
      },
      {
        key: "landClassification",
        label: "Land classification",
        weight: 0.06,
        scoreFn: "landcover",
        params: { mode: "developed" },
        description: "Previously developed, industrial, or barren land is favored.",
      },
      {
        key: "broadbandConnectivity",
        label: "Broadband readiness",
        weight: 0.1,
        scoreFn: "custom",
        params: { metric: "broadbandConnectivity", mode: "data_center" },
        description: "Uses FCC provider count and peak download/upload capacity as an early connectivity signal.",
      },
      {
        key: "floodRisk",
        label: "Flood risk",
        weight: 0.01,
        scoreFn: "custom",
        params: { metric: "floodRisk" },
        description: "Uses FEMA flood-zone designation to lightly penalize infrastructure risk in mapped SFHAs.",
      },
      {
        key: "contaminationRisk",
        label: "Contamination risk",
        weight: 0.01,
        scoreFn: "custom",
        params: { metric: "contaminationRisk" },
        description: "Uses EPA Superfund and TRI screening to lightly penalize nearby contamination context.",
      },
      {
        key: "groundwaterDepth",
        label: "Groundwater depth",
        weight: 0.02,
        scoreFn: "custom",
        params: { metric: "groundwaterDepth", mode: "data_center" },
        description: "Deeper water tables reduce dewatering and foundation complexity for infrastructure build-out.",
      },
      {
        key: "soilBuildability",
        label: "Soil buildability",
        weight: 0.02,
        scoreFn: "custom",
        params: { metric: "soilBuildability" },
        description: "Well-drained soils and stronger hydrologic groups support easier grading and foundation work.",
      },
      {
        key: "seismicRisk",
        label: "Seismic risk",
        weight: 0.01,
        scoreFn: "custom",
        params: { metric: "seismicRisk" },
        description: "Lower peak ground acceleration reduces structural reinforcement burden for major facilities.",
      },
      {
        key: "hazardAlerts",
        label: "Regional disaster alerts",
        weight: 0.01,
        scoreFn: "custom",
        params: { metric: "hazardAlerts" },
        description: "Uses GDACS live global disaster feed to lightly penalize sites in currently elevated-alert regions.",
      },
      {
        key: "terrainSlope",
        label: "Slope and ruggedness",
        weight: 0.06,
        scoreFn: "custom",
        params: { metric: "slopeRuggedness" },
        description: "Flatter, less rugged terrain reduces grading and foundation costs for construction.",
      },
      {
        key: "populationDensity",
        label: "Population density",
        weight: 0.04,
        scoreFn: "custom",
        params: { metric: "populationDensity", context: "general" },
        description: "Moderate population context helps identify industrial vs rural zones for infrastructure planning.",
      },
      {
        key: "landCoverGlobal",
        label: "Global land cover",
        weight: 0.03,
        scoreFn: "custom",
        params: { metric: "landCoverGlobal", context: "development_potential" },
        description: "Previously developed or bare land is favored; vegetated or water areas may require additional permitting.",
      },
      {
        key: "floodHazard",
        label: "Flood hazard",
        weight: 0.08,
        scoreFn: "custom",
        params: { metric: "floodHazard", context: "insurance" },
        description: "Flood risk affects insurance costs, downtime risk, and infrastructure protection requirements.",
      },
      {
        key: "droughtIndices",
        label: "Drought risk",
        weight: 0.05,
        scoreFn: "custom",
        params: { metric: "droughtIndices", context: "water_security" },
        description: "Drought severity impacts water availability for cooling and utility reliability.",
      },
      {
        key: "seismicHazard",
        label: "Seismic hazard",
        weight: 0.10,
        scoreFn: "custom",
        params: { metric: "seismicHazard", context: "engineering" },
        description: "Seismic acceleration affects structural design requirements, equipment bracing, and operational resilience.",
      },
    ],
    systemPrompt:
      "You are a geospatial intelligence expert focused on data center cooling and digital infrastructure siting. Prioritize water access, power grid adjacency, terrain buildability, climate suitability, and environmental constraints. Be explicit about what is supported by geodata versus what still needs permitting, engineering, or utility diligence.",
    defaultLayers: { water: true, power: true, roads: true, heatmap: false, buildings: false },
    exampleQuestions: [
      "Can this site support evaporative cooling?",
      "How far is the nearest substation or transmission corridor?",
      "What cooling risks stand out here?",
      "Would this work for a hyperscale campus?",
    ],
    recommendationBands: [
      { min: 85, text: "Excellent data-center cooling candidate with strong infrastructure alignment." },
      { min: 70, text: "Promising cooling site with a few constraints to validate in due diligence." },
      { min: 55, text: "Potentially viable only if permitting, grading, or utility issues are solved." },
      { min: 0, text: "Weak cooling-site fit compared with stronger regional benchmarks." },
    ],
  },
  {
    id: "hiking",
    name: getLensLabel("hiking"),
    icon: "Trees",
    tagline: "Trails, views, water, and remoteness",
    description:
      "Use this lens to evaluate day-hike or outdoor recreation potential based on terrain variety, vegetation, water features, access, and climate comfort.",
    accentColor: "#5be49b",
    factors: [
      {
        key: "terrainVariety",
        label: "Terrain variety",
        weight: 0.23,
        scoreFn: "custom",
        params: { metric: "terrainVariety" },
        description: "Moderate elevation gain and terrain variety improve hiking appeal.",
      },
      {
        key: "vegetationDensity",
        label: "Vegetation density",
        weight: 0.18,
        scoreFn: "landcover",
        params: { mode: "vegetation" },
        description: "Forested and vegetated landscapes generally increase recreation value.",
      },
      {
        key: "waterFeatures",
        label: "Water features nearby",
        weight: 0.19,
        scoreFn: "distance",
        params: { source: "water", idealKm: 0.5, cutoffKm: 12, direction: "near" },
        description: "Waterfalls, rivers, lakes, and streams increase trail destination value.",
      },
      {
        key: "remoteness",
        label: "Distance from urban areas",
        weight: 0.15,
        scoreFn: "custom",
        params: { metric: "remoteness" },
        description: "A modest buffer from major roads can improve the feeling of remoteness.",
      },
      {
        key: "trailAccess",
        label: "Trail access",
        weight: 0.08,
        scoreFn: "custom",
        params: { metric: "trailAccess" },
        description: "Blends OSM-mapped trailhead and hiking-path counts with road proximity for day-use logistics.",
      },
      {
        key: "weather",
        label: "Climate / weather",
        weight: 0.09,
        scoreFn: "climate",
        params: { mode: "outdoor" },
        description: "Moderate temperatures and manageable precipitation support broader usability.",
      },
      {
        key: "airQuality",
        label: "Air quality",
        weight: 0.08,
        scoreFn: "custom",
        params: { metric: "airQuality" },
        description: "Uses OpenAQ PM2.5 and PM10 station readings, with Open-Meteo AQI as a fallback context signal.",
      },
      {
        key: "terrainSlope",
        label: "Slope and ruggedness",
        weight: 0.15,
        scoreFn: "custom",
        params: { metric: "slopeRuggedness" },
        description: "Varied terrain and slope changes increase trail interest and hiking challenge appeal.",
      },
      {
        key: "populationDensity",
        label: "Population density / remoteness",
        weight: 0.08,
        scoreFn: "custom",
        params: { metric: "populationDensity", context: "rural_preference" },
        description: "Lower population density indicates more remote, wilderness-like areas preferred for hiking.",
      },
      {
        key: "landCoverGlobal",
        label: "Global land cover type",
        weight: 0.10,
        scoreFn: "custom",
        params: { metric: "landCoverGlobal", context: "forest_coverage" },
        description: "Forest, shrubland, and natural vegetation provide scenic value and trail connectivity for hiking.",
      },
      {
        key: "floodHazard",
        label: "Flood hazard",
        weight: 0.06,
        scoreFn: "custom",
        params: { metric: "floodHazard", context: "general" },
        description: "High flood risk makes trails unsafe and unreliable during rainy seasons.",
      },
      {
        key: "droughtIndices",
        label: "Drought and water",
        weight: 0.08,
        scoreFn: "custom",
        params: { metric: "droughtIndices", context: "general" },
        description: "Severe drought impacts water availability on trails and in natural water sources.",
      },
      {
        key: "seismicHazard",
        label: "Seismic stability",
        weight: 0.04,
        scoreFn: "custom",
        params: { metric: "seismicHazard", context: "general" },
        description: "High seismic hazard increases rockfall, landslide, and trail instability risks.",
      },
    ],
    systemPrompt:
      "You are a geospatial intelligence expert focused on hiking, trail planning, and outdoor recreation suitability. Emphasize terrain character, water features, vegetation, access, remoteness, and weather comfort. When uncertainty exists, make it clear whether you are using direct geodata, live mapped nearby places, or inference about scenic and recreation value.",
    defaultLayers: { water: true, power: false, roads: true, heatmap: true, buildings: false },
    exampleQuestions: [
      "What's the terrain difficulty here?",
      "Are there water features for a day hike?",
      "How remote does this area feel?",
      "Would this support a scenic trail network?",
    ],
    recommendationBands: [
      { min: 85, text: "Outstanding outdoor recreation candidate with strong terrain and scenic signals." },
      { min: 70, text: "Promising hiking area with a few access or climate tradeoffs." },
      { min: 55, text: "Usable for recreation, but not a standout trail destination without more validation." },
      { min: 0, text: "Limited recreation appeal compared with stronger hiking candidates." },
    ],
  },
  {
    id: "home-buying",
    name: getLensLabel("home-buying"),
    icon: "House",
    tagline: "Neighborhood fit, schools, and daily-life access",
    description:
      "Screen neighborhoods with a consumer-first lens that emphasizes schools, amenities, connectivity, environmental quality, and day-to-day livability.",
    accentColor: "#ffab00",
    factors: [
      {
        key: "schoolAccess",
        label: "School access",
        weight: 0.2,
        scoreFn: "custom",
        params: { metric: "schoolAccess" },
        description: "Uses nearby public-school context and official Washington accountability where available.",
      },
      {
        key: "amenities",
        label: "Daily amenities",
        weight: 0.18,
        scoreFn: "custom",
        params: { metric: "amenities" },
        description: "Uses live mapped grocery, restaurant, and service density as an everyday convenience signal.",
      },
      {
        key: "roadTransit",
        label: "Commute and access",
        weight: 0.15,
        scoreFn: "distance",
        params: { source: "road", idealKm: 1.5, cutoffKm: 14, direction: "near" },
        description: "Closer road access supports commuting and daily mobility.",
      },
      {
        key: "broadbandConnectivity",
        label: "Internet quality",
        weight: 0.12,
        scoreFn: "custom",
        params: { metric: "broadbandConnectivity", mode: "residential" },
        description: "Uses FCC provider count and speeds as a work-from-home and streaming readiness signal.",
      },
      {
        key: "floodRisk",
        label: "Flood risk",
        weight: 0.12,
        scoreFn: "custom",
        params: { metric: "floodRisk" },
        description: "Uses FEMA flood-zone designation as an early caution signal where supported.",
      },
      {
        key: "airQuality",
        label: "Air quality",
        weight: 0.1,
        scoreFn: "custom",
        params: { metric: "airQuality" },
        description: "Uses station readings and AQI fallback to screen for everyday environmental quality.",
      },
      {
        key: "buildability",
        label: "Terrain practicality",
        weight: 0.06,
        scoreFn: "elevation",
        params: { mode: "buildability" },
        description: "Moderate terrain is easier for day-to-day access and future maintenance.",
      },
      {
        key: "landClassification",
        label: "Neighborhood land mix",
        weight: 0.04,
        scoreFn: "landcover",
        params: { mode: "residential" },
        description: "Favors mixed residential and urban land cover over heavily constrained surfaces.",
      },
      {
        key: "contaminationRisk",
        label: "Environmental caution",
        weight: 0.03,
        scoreFn: "custom",
        params: { metric: "contaminationRisk" },
        description: "Uses EPA screening to flag unusually close contamination context where supported.",
      },
      {
        key: "terrainSlope",
        label: "Slope and ruggedness",
        weight: 0.08,
        scoreFn: "custom",
        params: { metric: "slopeRuggedness" },
        description: "Flatter terrain reduces drainage and grading costs, improves walkability and land value stability.",
      },
      {
        key: "populationDensity",
        label: "Population density",
        weight: 0.06,
        scoreFn: "custom",
        params: { metric: "populationDensity", context: "general" },
        description: "Moderate to moderate-low population density supports family living without excessive urban density.",
      },
      {
        key: "landCoverGlobal",
        label: "Global land cover",
        weight: 0.07,
        scoreFn: "custom",
        params: { metric: "landCoverGlobal", context: "general" },
        description: "Land cover context helps identify neighborhood character—residential vs rural, proximity to parks or water.",
      },
      {
        key: "floodHazard",
        label: "Flood risk",
        weight: 0.10,
        scoreFn: "custom",
        params: { metric: "floodHazard", context: "insurance" },
        description: "Flood risk directly affects home insurance costs, resale value, and family safety.",
      },
      {
        key: "droughtIndices",
        label: "Drought and water stress",
        weight: 0.08,
        scoreFn: "custom",
        params: { metric: "droughtIndices", context: "agricultural" },
        description: "Drought conditions impact water restrictions, utility costs, and landscape maintenance.",
      },
      {
        key: "seismicHazard",
        label: "Seismic risk",
        weight: 0.08,
        scoreFn: "custom",
        params: { metric: "seismicHazard", context: "insurance" },
        description: "Seismic hazard affects earthquake insurance costs and structural retrofit requirements.",
      },
    ],
    systemPrompt:
      "You are a geospatial intelligence assistant focused on home buying and neighborhood screening. Prioritize school access, daily amenities, commute practicality, internet quality, flood exposure, air quality, and plain-language livability signals. Speak clearly for ordinary buyers, keep technical jargon behind short explanations, and state openly when a signal is unsupported or only approximate.",
    defaultLayers: { water: true, power: false, roads: true, heatmap: false, buildings: false },
    exampleQuestions: [
      "Does this feel like a strong place to buy a home?",
      "How good is the school and amenity access here?",
      "What daily-life risks or tradeoffs stand out?",
      "Would this work well for a family moving here?",
    ],
    recommendationBands: [
      { min: 85, text: "Strong home-buying signal with good access, livability, and few obvious early risks." },
      { min: 70, text: "Promising neighborhood with a few tradeoffs worth checking more closely." },
      { min: 55, text: "Potentially workable for home buying, but some quality-of-life or risk questions remain." },
      { min: 0, text: "Weak home-buying fit compared with stronger nearby neighborhoods." },
    ],
  },
  {
    id: "site-development",
    name: getLensLabel("site-development"),
    icon: "Building2",
    tagline: "Buildability, hazards, and development readiness",
    description:
      "Evaluate land for neighborhood-scale residential growth using access, buildability, hazard proxies, amenity signals, and general development readiness.",
    accentColor: "#ffab00",
    factors: [
      {
        key: "schoolAccess",
        label: "School district proximity",
        weight: 0.15,
        scoreFn: "custom",
        params: { metric: "schoolAccess" },
        description: "Uses nearby public-school context from NCES and Washington official accountability when available.",
      },
      {
        key: "roadTransit",
        label: "Road / transit access",
        weight: 0.13,
        scoreFn: "distance",
        params: { source: "road", idealKm: 1.5, cutoffKm: 14, direction: "near" },
        description: "Closer access supports commuting and neighborhood connectivity.",
      },
      {
        key: "buildability",
        label: "Terrain buildability",
        weight: 0.12,
        scoreFn: "elevation",
        params: { mode: "buildability" },
        description: "Moderate, more buildable terrain scores higher.",
      },
      {
        key: "floodRisk",
        label: "Flood risk",
        weight: 0.13,
        scoreFn: "custom",
        params: { metric: "floodRisk" },
        description: "Uses FEMA flood-zone designation so Zone X carries no penalty while mapped SFHAs create an approximate fifteen-point total-score penalty.",
      },
      {
        key: "amenities",
        label: "Commercial amenities nearby",
        weight: 0.09,
        scoreFn: "custom",
        params: { metric: "amenities" },
        description: "Uses live mapped amenity density from OpenStreetMap.",
      },
      {
        key: "landClassification",
        label: "Land classification",
        weight: 0.09,
        scoreFn: "landcover",
        params: { mode: "residential" },
        description: "Avoids water-heavy land and favors mixed open or already-developed parcels.",
      },
      {
        key: "broadbandConnectivity",
        label: "Broadband readiness",
        weight: 0.07,
        scoreFn: "custom",
        params: { metric: "broadbandConnectivity", mode: "residential" },
        description: "Uses FCC provider count and peak speeds as a residential connectivity factor.",
      },
      {
        key: "airQuality",
        label: "Air quality",
        weight: 0.08,
        scoreFn: "custom",
        params: { metric: "airQuality" },
        description: "Uses OpenAQ particle readings, with Open-Meteo AQI fallback, as an environmental quality signal.",
      },
      {
        key: "contaminationRisk",
        label: "Contamination risk",
        weight: 0.06,
        scoreFn: "custom",
        params: { metric: "contaminationRisk" },
        description: "Uses EPA Superfund and TRI proximity as an early contamination-screening factor.",
      },
      {
        key: "groundwaterDepth",
        label: "Groundwater depth",
        weight: 0.03,
        scoreFn: "custom",
        params: { metric: "groundwaterDepth", mode: "residential" },
        description: "Moderate water-table depth is generally easier for housing foundations and yard drainage.",
      },
      {
        key: "soilBuildability",
        label: "Soil buildability",
        weight: 0.03,
        scoreFn: "custom",
        params: { metric: "soilBuildability" },
        description: "Well-drained soils with stronger hydrologic groups improve neighborhood buildability and drainage resilience.",
      },
      {
        key: "seismicRisk",
        label: "Seismic risk",
        weight: 0.01,
        scoreFn: "custom",
        params: { metric: "seismicRisk" },
        description: "Lower site shaking potential improves early residential risk screening.",
      },
      {
        key: "hazardAlerts",
        label: "Regional disaster alerts",
        weight: 0.01,
        scoreFn: "custom",
        params: { metric: "hazardAlerts" },
        description: "Uses GDACS live global disaster feed to lightly penalize sites in currently elevated-alert regions.",
      },
      {
        key: "terrainSlope",
        label: "Slope and ruggedness",
        weight: 0.10,
        scoreFn: "custom",
        params: { metric: "slopeRuggedness" },
        description: "Flatter terrain reduces grading and infrastructure costs, improving neighborhood development feasibility.",
      },
      {
        key: "populationDensity",
        label: "Population density",
        weight: 0.05,
        scoreFn: "custom",
        params: { metric: "populationDensity", context: "general" },
        description: "Population context helps identify growth potential and existing neighborhood character.",
      },
      {
        key: "landCoverGlobal",
        label: "Global land cover",
        weight: 0.06,
        scoreFn: "custom",
        params: { metric: "landCoverGlobal", context: "development_potential" },
        description: "Bare, agricultural, or previously developed land is more suitable for new residential development.",
      },
      {
        key: "floodHazard",
        label: "Flood risk",
        weight: 0.08,
        scoreFn: "custom",
        params: { metric: "floodHazard", context: "development" },
        description: "Flood hazard increases infrastructure costs, regulatory burden, and resident safety concerns.",
      },
      {
        key: "droughtIndices",
        label: "Drought and water stress",
        weight: 0.07,
        scoreFn: "custom",
        params: { metric: "droughtIndices", context: "agricultural" },
        description: "Drought conditions impact water supply reliability and landscape maintenance viability.",
      },
      {
        key: "seismicHazard",
        label: "Seismic hazard",
        weight: 0.09,
        scoreFn: "custom",
        params: { metric: "seismicHazard", context: "engineering" },
        description: "Seismic acceleration increases building code requirements and construction costs.",
      },
    ],
    systemPrompt:
      "You are a geospatial intelligence expert focused on residential site development and neighborhood planning. Emphasize buildability, access, hazards, school context, amenity context, and community readiness. Distinguish official government school accountability data from GeoSight's derived school-context score, and state clearly when school coverage is unsupported outside the current US-first pipeline.",
    defaultLayers: { water: true, power: true, roads: true, heatmap: false, buildings: false },
    exampleQuestions: [
      "Is this land suitable for a new neighborhood?",
      "What risks or constraints would slow housing here?",
      "How accessible is this to nearby services?",
      "Does this feel more suburban or rural?",
    ],
    recommendationBands: [
      { min: 85, text: "Strong residential site-development candidate with good access and few obvious buildability issues." },
      { min: 70, text: "Promising housing site with some hazard or amenity tradeoffs to review." },
      { min: 55, text: "Possible residential site, but key infrastructure or risk constraints remain unresolved." },
      { min: 0, text: "Weak neighborhood-development fit without major infrastructure or entitlement changes." },
    ],
  },
  {
    id: "energy-solar",
    name: "Energy & Solar",
    icon: "Zap",
    tagline: "Solar resource, wind potential, and grid access",
    description:
      "Evaluate sites for solar farms, wind projects, and distributed energy with emphasis on renewable resource, grid proximity, and land suitability.",
    accentColor: "#f59e0b",
    factors: [
      {
        key: "climate",
        label: "Solar irradiance",
        weight: 0.30,
        scoreFn: "climate",
        params: { mode: "outdoor" },
        description: "Sunny, dry climates with high irradiance days improve energy yield potential.",
      },
      {
        key: "powerInfrastructure",
        label: "Grid / transmission access",
        weight: 0.25,
        scoreFn: "distance",
        params: { source: "power", idealKm: 3, cutoffKm: 25, direction: "near" },
        description: "Proximity to substations and transmission corridors reduces interconnection cost.",
      },
      {
        key: "terrain",
        label: "Terrain flatness",
        weight: 0.18,
        scoreFn: "elevation",
        params: { mode: "buildability" },
        description: "Flatter terrain lowers installation cost and maximizes panel alignment.",
      },
      {
        key: "landClassification",
        label: "Land use suitability",
        weight: 0.15,
        scoreFn: "landcover",
        params: { mode: "developed" },
        description: "Open, barren, or previously disturbed land is preferred over dense forest or wetland.",
      },
      {
        key: "transportation",
        label: "Construction access",
        weight: 0.12,
        scoreFn: "distance",
        params: { source: "road", idealKm: 2, cutoffKm: 20, direction: "near" },
        description: "Road access supports equipment delivery and ongoing operations and maintenance.",
      },
    ],
    systemPrompt:
      "You are a geospatial intelligence expert focused on renewable energy siting — solar farms, wind projects, and distributed energy. Evaluate solar irradiance, wind exposure, grid proximity, land use suitability, and construction access. Be explicit when irradiance or wind resource data is derived from climate proxies rather than measured resource datasets.",
    defaultLayers: { water: false, power: true, roads: true, heatmap: false, buildings: false },
    exampleQuestions: [
      "How strong is the solar resource here?",
      "Is this site viable for a utility-scale solar farm?",
      "How far is the nearest grid interconnection point?",
      "What land-use constraints might affect permitting?",
    ],
    recommendationBands: [
      { min: 85, text: "Excellent renewable energy candidate with strong resource and good grid access." },
      { min: 70, text: "Promising energy site with manageable infrastructure or land-use tradeoffs." },
      { min: 55, text: "Potential energy site, but resource, grid, or permitting questions need resolution." },
      { min: 0, text: "Weak energy siting fit relative to higher-resource alternatives." },
    ],
  },
  {
    id: "agriculture",
    name: "Agriculture & Land",
    icon: "Leaf",
    tagline: "Soil quality, water access, and growing conditions",
    description:
      "Evaluate land for farming, ranching, and agricultural investment with emphasis on soil quality, water proximity, climate suitability, and market access.",
    accentColor: "#22c55e",
    factors: [
      {
        key: "soilBuildability",
        label: "Soil quality",
        weight: 0.30,
        scoreFn: "custom",
        params: { metric: "soilBuildability" },
        description: "Well-drained soils with good hydrologic group ratings support more crop types and higher yields.",
      },
      {
        key: "waterProximity",
        label: "Water access",
        weight: 0.25,
        scoreFn: "custom",
        params: { metric: "waterAccess" },
        description: "Proximity to rivers, streams, and aquifer recharge areas is critical for irrigation.",
      },
      {
        key: "climate",
        label: "Growing climate",
        weight: 0.20,
        scoreFn: "climate",
        params: { mode: "outdoor" },
        description: "Favorable temperature range and precipitation support longer growing seasons.",
      },
      {
        key: "terrain",
        label: "Topography",
        weight: 0.15,
        scoreFn: "elevation",
        params: { mode: "buildability" },
        description: "Gently sloping or flat terrain minimizes erosion risk and equipment operational costs.",
      },
      {
        key: "transportation",
        label: "Market access",
        weight: 0.10,
        scoreFn: "distance",
        params: { source: "road", idealKm: 5, cutoffKm: 30, direction: "near" },
        description: "Road access to transport corridors reduces post-harvest logistics costs.",
      },
    ],
    systemPrompt:
      "You are a geospatial intelligence expert focused on agricultural land evaluation and rural land investment. Evaluate soil quality, water access, climate suitability, topography, and market proximity. Be explicit when soil data is derived from SSURGO hydrology groups rather than direct agronomic testing. Note when water-access signals come from stream-proximity proxies rather than confirmed water rights.",
    defaultLayers: { water: true, power: false, roads: true, heatmap: false, buildings: false },
    exampleQuestions: [
      "Is this land suitable for row crops or row farming?",
      "What does the soil and water access look like here?",
      "How does this compare to nearby agricultural land?",
      "What growing season constraints should I plan around?",
    ],
    recommendationBands: [
      { min: 85, text: "Strong agricultural site with good soil, water access, and favorable growing conditions." },
      { min: 70, text: "Viable farming or ranching land with some soil, water, or access tradeoffs." },
      { min: 55, text: "Marginal agricultural fit — soil, water, or climate constraints may limit productivity." },
      { min: 0, text: "Poor agricultural fit without significant remediation or irrigation investment." },
    ],
  },
  {
    id: "emergency-response",
    name: "Emergency Response",
    icon: "ShieldAlert",
    tagline: "Access routes, population exposure, and facility gaps",
    description:
      "Analyze areas for emergency preparedness, disaster response staging, and critical facility placement with emphasis on road access, population reach, hazard concentration, and infrastructure gaps.",
    accentColor: "#ef4444",
    factors: [
      {
        key: "transportation",
        label: "Road / access network",
        weight: 0.30,
        scoreFn: "distance",
        params: { source: "road", idealKm: 1, cutoffKm: 8, direction: "near" },
        description: "Dense road networks and low travel times to arterials are essential for response coverage.",
      },
      {
        key: "trafficPopulation",
        label: "Population concentration",
        weight: 0.25,
        scoreFn: "custom",
        params: { metric: "commercialDemand" },
        description: "High population-density areas demand greater emergency resource proximity.",
      },
      {
        key: "floodRisk",
        label: "Hazard exposure",
        weight: 0.20,
        scoreFn: "custom",
        params: { metric: "floodRisk" },
        description: "Flood, fire, and seismic hazard concentration drives response priority and staging needs.",
      },
      {
        key: "broadbandConnectivity",
        label: "Communications readiness",
        weight: 0.15,
        scoreFn: "custom",
        params: { metric: "broadbandConnectivity", mode: "data_center" },
        description: "Broadband and cell coverage density supports real-time coordination and dispatch.",
      },
      {
        key: "powerInfrastructure",
        label: "Critical infrastructure",
        weight: 0.10,
        scoreFn: "distance",
        params: { source: "power", idealKm: 2, cutoffKm: 15, direction: "near" },
        description: "Proximity to power substations and utility corridors affects facility resilience.",
      },
    ],
    systemPrompt:
      "You are a geospatial intelligence expert focused on emergency preparedness and disaster response analysis. Evaluate road access, population exposure, hazard concentration, communications readiness, and critical infrastructure proximity. Be explicit when population data comes from land-cover and road-density proxies rather than census counts. Note when hazard signals are FEMA zone designations versus live event data.",
    defaultLayers: { water: true, power: true, roads: true, heatmap: false, buildings: false },
    exampleQuestions: [
      "How accessible is this area for emergency vehicles?",
      "Where are the biggest hazard exposure gaps?",
      "Is communications infrastructure reliable here?",
      "What population center is hardest to reach from this location?",
    ],
    recommendationBands: [
      { min: 85, text: "Well-connected area with strong access, low hazard exposure, and good infrastructure support." },
      { min: 70, text: "Adequate response posture with some access, hazard, or communications gaps to address." },
      { min: 55, text: "Moderate preparedness gaps — access, hazard, or communications constraints are significant." },
      { min: 0, text: "High-risk emergency management context with critical access or infrastructure deficits." },
    ],
  },
  {
    id: "field-research",
    name: "Field Research",
    icon: "FlaskConical",
    tagline: "Habitat quality, ecological diversity, and site access",
    description:
      "Evaluate field sites for ecological research, environmental monitoring, and conservation assessment with emphasis on habitat quality, topographic diversity, climate stability, and access.",
    accentColor: "#8b5cf6",
    factors: [
      {
        key: "landClassification",
        label: "Habitat & land cover",
        weight: 0.30,
        scoreFn: "landcover",
        params: { mode: "natural" },
        description: "Natural land cover (forest, wetland, grassland) indicates ecological richness for field study.",
      },
      {
        key: "waterProximity",
        label: "Aquatic features",
        weight: 0.25,
        scoreFn: "custom",
        params: { metric: "waterAccess" },
        description: "Proximity to rivers, lakes, and wetlands supports higher biodiversity and site connectivity.",
      },
      {
        key: "terrain",
        label: "Topographic diversity",
        weight: 0.20,
        scoreFn: "elevation",
        params: { mode: "outdoor" },
        description: "Varied terrain supports higher species diversity and more complex habitat mosaics.",
      },
      {
        key: "climate",
        label: "Climate stability",
        weight: 0.15,
        scoreFn: "climate",
        params: { mode: "outdoor" },
        description: "Moderate, stable climate conditions support long-term monitoring continuity.",
      },
      {
        key: "transportation",
        label: "Field access",
        weight: 0.10,
        scoreFn: "distance",
        params: { source: "road", idealKm: 3, cutoffKm: 25, direction: "near" },
        description: "Road proximity provides reasonable field access while allowing undisturbed habitat.",
      },
    ],
    systemPrompt:
      "You are a geospatial intelligence expert focused on ecological field research and environmental monitoring site selection. Evaluate habitat quality, ecological diversity indicators, topographic complexity, climate stability, and field access. Be explicit when habitat data is inferred from land-cover classification rather than species occurrence records. Note protected land designations when available.",
    defaultLayers: { water: true, power: false, roads: true, heatmap: false, buildings: false },
    exampleQuestions: [
      "What ecological features make this site interesting for research?",
      "How accessible is this site for field crews?",
      "What habitat types are present in this area?",
      "How does this compare to nearby reference sites?",
    ],
    recommendationBands: [
      { min: 85, text: "Excellent field research site with diverse habitat, good access, and stable climate." },
      { min: 70, text: "Strong research candidate with some access, habitat, or climate considerations." },
      { min: 55, text: "Moderate site quality — significant access, habitat, or climate constraints present." },
      { min: 0, text: "Challenging field research context with major access or habitat quality limitations." },
    ],
  },
  {
    id: "commercial",
    name: getLensLabel("commercial"),
    icon: "Warehouse",
    tagline: "Access, demand, and utility readiness",
    description:
      "Use this profile for retail, commercial, and warehouse siting questions tied to highway access, inferred commercial activity, utility proximity, and development practicality.",
    accentColor: "#a78bfa",
    factors: [
      {
        key: "trafficPopulation",
        label: "Traffic / population density",
        weight: 0.25,
        scoreFn: "custom",
        params: { metric: "commercialDemand" },
        description: "Uses road access and urbanized land-cover as an early demand proxy.",
      },
      {
        key: "freightAccess",
        label: "Highway / freight access",
        weight: 0.2,
        scoreFn: "distance",
        params: { source: "road", idealKm: 1.2, cutoffKm: 12, direction: "near" },
        description: "Closer highway corridors improve logistics and commercial throughput.",
      },
      {
        key: "commercialDensity",
        label: "Existing commercial density",
        weight: 0.15,
        scoreFn: "custom",
        params: { metric: "commercialDensity" },
        description: "Uses developed land-cover, roads, and utilities as a proxy for existing activity.",
      },
      {
        key: "landCostIndicators",
        label: "Land cost indicators",
        weight: 0.15,
        scoreFn: "custom",
        params: { metric: "landCost" },
        description: "Uses distance from core infrastructure and land-cover mix as an early cost heuristic.",
      },
      {
        key: "utilities",
        label: "Utility infrastructure",
        weight: 0.15,
        scoreFn: "distance",
        params: { source: "power", idealKm: 2, cutoffKm: 18, direction: "near" },
        description: "Commercial and warehouse operations benefit from nearby utility corridors.",
      },
      {
        key: "terrain",
        label: "Terrain flatness",
        weight: 0.1,
        scoreFn: "elevation",
        params: { mode: "buildability" },
        description: "Flatter sites reduce grading complexity for commercial build-out.",
      },
    ],
    systemPrompt:
      "You are a geospatial intelligence expert focused on retail, commercial real estate, and warehouse siting. Prioritize freight and road access, inferred demand, utility readiness, and land practicality. Be explicit when demand, traffic, or land-cost commentary is based on proxy signals rather than live demographic or parcel data. When analyzing locations outside the US, note that demographic data comes from World Bank national indicators rather than county-level census data, and school context is not yet available.",
    defaultLayers: { water: false, power: true, roads: true, heatmap: false, buildings: false },
    exampleQuestions: [
      "How strong is the highway access here?",
      "Would this work for a distribution warehouse?",
      "Does this area look commercially active already?",
      "What operational constraints stand out for retail or logistics?",
    ],
    recommendationBands: [
      { min: 85, text: "Excellent commercial or logistics candidate with strong access and utility support." },
      { min: 70, text: "Promising commercial site with manageable market or site-readiness tradeoffs." },
      { min: 55, text: "Potential fit for commercial use, but important demand or build-out questions remain." },
      { min: 0, text: "Weak commercial / warehouse fit relative to stronger corridor candidates." },
    ],
  },
];

export const DEFAULT_PROFILE = PROFILES[0];

export const PROFILE_MAP = Object.fromEntries(PROFILES.map((profile) => [profile.id, profile])) as Record<
  string,
  MissionProfile
>;

export function getProfileById(profileId: string) {
  const normalizedProfileId = normalizeProfileId(profileId);
  return (normalizedProfileId ? PROFILE_MAP[normalizedProfileId] : undefined) ?? DEFAULT_PROFILE;
}
