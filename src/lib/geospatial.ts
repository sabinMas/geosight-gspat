...
export function toBoundingBox(center: Coordinates, radiusKm = 3): BoundingBox {
  ...
  const bboxPolygon = buffer(centerPoint, radiusKm, { units: "kilometers" })!;
  const [west, south, east, north] = bbox(bboxPolygon);
...
export function estimateRegionSpanKm(bbox: BoundingBox) {
  const westPoint = point([bbox.west, (bbox.north + bbox.south) / 2]);
  const eastPoint = point([bbox.east, (bbox.north + bbox.south) / 2]);
  const widthKm = distance(westPoint, eastPoint, { units: "kilometers" });
...
export function buildElevationTransect(
  ...
  const start = destination(centerPoint, halfLengthKm, bearing - 180, { units: "kilometers" });
  const end = destination(centerPoint, halfLengthKm, bearing, { units: "kilometers" });
  ...
  return Array.from({ length: samples }, (_, index) => {
    ...
    const sampled = along(line, segment, { units: "kilometers" });
